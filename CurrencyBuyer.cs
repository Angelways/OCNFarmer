...
internal sealed unsafe class CurrencyBuyer : IDisposable
{
    ...
    private Vector3 activeInitialCrystal = IslandProfile.North.InitialCrystal;
    ...
    private bool IsNearInitialCrystal(Vector3? position)
    {
        if (position == null)
            return false;

        var pos = position.Value;
        var dx = pos.X - activeInitialCrystal.X;
        var dz = pos.Z - activeInitialCrystal.Z;
