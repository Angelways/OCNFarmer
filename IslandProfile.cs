...
internal sealed class IslandProfile
{
    ...
    public required Vector3 InitialCrystal { get; init; }
    ...
    internal static IslandProfile North { get; } = new()
    {
        ...
        InitialCrystal = new(882f, 258.5f, 882f),
    ...
    internal static IslandProfile South { get; } = new()
    {
        ...
        InitialCrystal = new(834f, 73f, -694.6f),
        ShardKeywords = ["放浪", "洞窟", "古树", "石塔"],
    ...
    internal static IslandProfile? ForTerritory(uint territory) => territory switch
    {
        NorthTerritoryId => North,
        SouthTerritoryId => South,
        _ => null,
    };

    internal static IslandProfile Resolve(IslandTarget target, uint currentTerritory)
    {
        if (target == IslandTarget.NorthHorn) return North;
        if (target == IslandTarget.SouthHorn) return South;
        return ForTerritory(currentTerritory) ?? North;
    }

    internal static bool IsSupportedTerritory(uint territory) =>
        territory is NorthTerritoryId or SouthTerritoryId;
}
